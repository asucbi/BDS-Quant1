...
-   Go to [posit.cloud](posit.cloud) and then **navigate to the course workspace** via the left sidebar.
    ...
    -   Look for the course workspace called "Spring 2026: Statistical Methods"

-   Open the project titled **Module 13** This module contains the starter files that you will use for the lab and for your homework.

## Starter Files

In the Files pane you should see this R Notebook:

-   mod-13-lab-starter.qmd
...
You may also want to run `na.omit()` on the data so that you remove participants with missing data. 
...
## Q9: Generate an ICC to determine how much variance in looking time is explained by trial type. Report the ICC value and explain what it means.
...
## Q12: Interpret the model parameters for the fixed effects. 

answer Q12 here

## Q13: Interpret the random effects. How much variance is attributable to the random intercept for subjects? How much variance is left over?

answer Q13 here

## Q14: Now, let's test the influence of adding a random slope for trial type. Fit a model predicting `looking_time` from `trial_type` and `age_mo_centered` (no interaction). This time, include a random *intercept* for `subid` and a random *slope* for `trial_type`.
...
## Q15: Interpret the model parameters for the fixed effects. 

answer Q15 here

## Q16: Interpret the random effects. How much variance is attributable to the random intercept for subjects? ...the random slope for subjects? How much variance is left over?

answer Q16 here

## Q17: Can we include a random slope for age? Why or why not?

answer Q17 here

## Q18: Let's determine whether the random intercept and slope model fits the data better than the model with just a random intercept. To do this, use the `test_likelihoodratio()` function from the `{performance}` package.

```{r likelihood ratio 1 , echo=FALSE, message=FALSE}

lrt_1 <- test_likelihoodratio(model_1, model_2, REML = TRUE)
lrt_1

```

## Q19: Does the model that includes a random slope for `trial_type` improve model fit over the model with just a random intercept for `subid`? How do you know?

answer Q19 here

## Q20: Now, let's determine whether adding an interaction between `trial_type` and `age_mo_centered` improves model fit. Add the interaction term to the better fitting model (based on your likelihood ratio test above).
...
## Q21: Interpret the model parameters for the fixed effects. 

answer Q21 here

## Q22: Interpret the random effects. How much variance is attributable to the random intercept for subjects? How much variance is left over?

answer Q22 here

## Q23: Let's determine whether the model with the interaction term fits the data better than the model with just main effects. To do this, use the `test_likelihoodratio()` function from the `{performance}` package.

```{r likelihood ratio 2 , echo=FALSE, message=FALSE}

lrt_2 <- test_likelihoodratio(model_1, model_3, REML = FALSE)
lrt_2

```

## Q24: What is the best fitting model?

answer Q24 here

## Q25: What are the fixed effects in your final model?

answer Q25 here

## Q26: Generate effect sizes for the overall best fitting model and all of the fixed effects in your best fitting model (hint: you will use the `r2` and `partR2` functions as well as calculating a Cohen's d "by hand").
...
## Q27: Use the `check_model()` function from the `{performance}` package to check the assumptions of the best fitting model. How do these model assumptions look?
...
## Q28: Generate an APA Summary for the best fitting model. Make sure to include *all* necessary information (see Module 13 lecture slides and example shared on Canvas).
