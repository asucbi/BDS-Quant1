...
Employees are nested within companies. This matters because employees within the same company are likely more similar to each other than employees from different companies, so the observations are not independent.
...
The slope is **1.27**, so for each additional hour of sleep, performance increases by about **1.27 points**, on average. No, it is not significant (p = .31)

## Q4: What assumption about clustering / independence does this model make?

This model assumes that all observations are independent and that company-level clustering does not matter.
...
The slope is **-1.46**, so for each additional hour of sleep, performance decreases by about **1.46 points**, on average. It is not significant (p = .98)
...
Aggregation removes within-company variability, while disaggregation ignores between-company variability.
...
The ICC is **0.968**. That means about **96.79%** of the variance in performance is attributable to differences between companies. Company membership explains a very large amount of the variability in performance.
...
Both appear to vary. The lines start at different heights and have different steepness across companies.
...
`company_id` is significant in this model, which means companies differ in their baseline performance levels (their intercepts), even after accounting for sleep hours. The main effect of `sleep_hours` is also positive and significant.
...
The interaction model indicates that slopes differ across companies, and the interaction is significant overall. Adding the interaction improves fit substantially over the model with only main effects, so the relationship between sleep and performance is not the same for all companies.
...
`company_size` significantly predicts intercepts in this dataset meaning that company size explains baseline performance differences here.
...
`company_size` significantly predict slopes in this dataset. That means company size explains the differences in the sleep–performance relationship across companies.
...
-   Submit the rendered file to Canvas
