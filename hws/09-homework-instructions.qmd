...
2.  Use `lm()` to perform a factorial ANOVA analysis as *regression.* Use functions from the `{broom}` package (as we did in lab) to create a clean table of the coefficients. Make sure to include CIs.
...
4.  Use the `eta_squared` function from the `{effectsize}` package to generate effect sizes for each of the effects in the model. For each effect (rejection sensitivity, partner, interaction) report the effects size and whether the effect size is small, medium, or large.
...
```{r message = FALSE, echo = FALSE, eval = FALSE}
emmeans::emmeans(model, pairwise ~rs, adjust="bonferroni")$contrasts 
emmeans::emmeans(model, pairwise ~partner, adjust="bonferroni")$contrasts 
emmeans::emmeans(model, pairwise~rs|partner)
...
7.  Use the `F_to_eta2` function to generate a partial eta-squared effect size for the effect of `rs` at each level of `partner`. For each effect (e.g., rejection sensitivity for happy partner, rejection sensitivity for neutral partner) report whether the effect size is small, medium, or large.

```{r message = FALSE, echo = FALSE, eval = FALSE}

F_to_eta2(f = c(217.668, 3.134), 
          df=c(1,1), 
...
2.  Use the `power_plot` function to plot power with a sample size ranging from 5 to 50. This may take a little while to run!
...
3.  How many *total participants* (not per condition!) would we need to achieve over 90% power for the main effects and interaction?
