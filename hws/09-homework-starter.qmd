...
#make sure to set the seed to 505 so that your answers match the key!
set.seed(505)
...
7.  Use the `F_to_eta2` function to generate a partial eta-squared effect size for the effect of `rs` at each level of `partner`. For each effect (e.g., rejection sensitivity for a neutral partner, rejection sensitivity for a happy partner) report whether the effect size is small, medium, or large.
