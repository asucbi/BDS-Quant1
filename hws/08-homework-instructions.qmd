...
library(performance) #for checking multicollinearity
...
1.  Run a model testing whether *test anxiety* moderates the relationship between time spent studying and test performance. Take steps to reduce multicollinearity *before* running the model (think: what can I do *before running the model* to reduce multicollinearity).
...
2.  Write out the full model with parameter estimates filled in. Interpret each of the model coefficients (parameters).
...
6.  Write an APA style summary of your results. Use the example from class to guide your summary.

7.  Answer the questions: (a) What contrast codes were applied to the `anxiety` factor in the model you ran? (b) How would your interpretation of each of the coefficients change if you instead assigned these contrasts: -0.5 = low, 0.5 = high? No need to re-run the model, I just want your explanation.

## Exercise 2:

1.  Now, run a model testing whether students' *previous test score* moderates the relationship between time spent studying and test performance. Take the same steps to reduce multicollinearity before running the model.
...
7.  Write an APA style summary of your results. Use the example from class to guide your summary.
