...
> Question: What kind of ANOVA is this (\_\_\_\_\_-way)? How would you further describe the design?

ANSWER: it is a two-way ANOVA because there are two factors (drug and dose) and a 2 x 3 design because 2 levels of drug and 3 levels of dose.
...
# model1 <- lm(XXX ~ XXX * XXX, data = pain_red)
# summary(model1)
...
Interaction: interaction between drug and dose
...
lm(pain_level ~ drug * dose, data = pain) %>%
  emmeans::emmeans(pairwise~dose|drug) %>%
  joint_tests(by="drug") %>% 
  kable() %>% 
  kable_styling()
...
emmeans::emmeans(model3, pairwise ~dose, adjust="bonferroni")$contrasts %>% 
  kable() %>% 
  kable_styling()
...
ANSWER: Drug: 39%, Dose: 100%, Interaction: 89%
