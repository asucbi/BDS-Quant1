...
sample_5 <- rep_sample_n(gapminder_2015, size = 5, reps = 10000) %>% #here we are getting 100 samples of size 5
  ...
  summarise(mean_infant_mortality = mean(life_expectancy)) %>% #now, get the mean for each sample
  mutate(n = "5") #add a variable that indcates the sample size
...
# then, I'll use those objects to calcualte the SEM
...
We examined the effect of an 8-week mindfulness meditation program on stress levels.The average score of participants after the program (*M* = `r sample_mean`, 95%CI \[`r lower_bound`, `r upper_bound`\], *SD* = `r estimated_sigma`) was significantly higher than the population average ($\mu$ = `r population_mean`), *t*(`r df_value`) = `r round(t_statistic, 2)`, *p* = `r round(p_value, 3)`.This finding suggests that the mindfulness meditation program has a significant impact on reducing stress levels in participants.
...
I would conclude that there is not a signficiant effect. The mindfulness program did not decrease stress more than would be expected by chance.

If the observed sample mean was 82, the t statistic would be 2.92 and p-value would be .008. What do you conclude?

I would conclude that there IS a signficant effect. BUT the mindfulness program actually INCREASED stress levels. Oops! Even though this wasn't your prediction, it would still be useful to know!
