---
title: "Module 4 Lab: Starter File KEY"
format: html
editor: visual
---
...
In today's lab, we will review Multiple Regression, work with Partial Correlations, and practice significance testing for Multiple Regression.
...
#students: fill in XXX with correct code
...
ANS: Both age and height seem to be positively correlated with weight.
...
```{r run-regression, message = FALSE}

#students: type out this model
...
ANS: When age and height are 0 the predicted weight is -141.22. For each one-unit increase in age we expect weight to increase by 1.28, holding height constant. For each one-unit increase in height we expect weight to increase by 3.60, holding age constant.
...
```{r age~height-resid, message= FALSE}
...
3.  Regress `weight` on the part of `age` that is unrelated to `height` (i.e., `age_resid`). Store the model in an object called `model_3`.

```{r weight~age_resid, message= FALSE}
...
ANS: It's the same thing!

# Partial Correlations with the {ppcor} package.

Here, we will calculate the **partial** correlation for age using the `pcor.test` function.

```{r p-cor, message= FALSE}
...
```{r intercept-only, message= FALSE}
...
```{r mod1and2-compare, message= FALSE}
...
ANS: Yes, height and age seem likely to be correlated.
...
```{r check-multicollinaerity, message = FALSE}
...
```{r calc-multicollinaerity, message = FALSE}
...
\*Note: Every time I try to put a bracket, R adds an extra backslash, so it looks like this: \[ or this \]. Remove the backslash before knitting or rendering the file.
