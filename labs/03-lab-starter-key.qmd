...
Once you have a correlation, you may want to assess whether it's statistically meaningful.

What is the null hypotehsis for a correlation?
...
#pause for students to fill in the blanks

A significant positive correlation was found between conscientiousness and self-reported health status, *r*(`r r_consc_health$n-2`) = `r round(r_consc_health$ci$r, 2)`, 95% CI\[`r round(r_consc_health$ci$lower,2)`, `r round(r_consc_health$ci$upper,2)`\], *p* `r papaja::printp(r_consc_health$p.adj)`. As conscientiousness status increases self-reported health status also increases.
...
The fitted values of Y are stored in `model` in an element called `fitted.values`. Using the instructions about how to extract elements from lists above, extract the fitted values from the model and store these in an object called `fitted_y`. Print `fitted_y`.
