...
Create a new dataset called `cat_reduced` that includes only the treatment groups. Use `View()` to double check that the data look correct.
...
ANSWER: Whether any mean is different from any other mean. So we already know there's a difference between the two groups.
...
> **Question:** Using the resulting equation, determine the mean depression score of each group.
...
ANSWER: 7.2: CBT 7.2 - 3.8 = 3.4: psychotherapy
...
ANSWER: mean of CBT is significantly different from 0.
...
ANSWER: The mean of psychotherapy is significantly different from the mean of CBT.
...
ANSWER: mean of Psychotherapy is significantly different from 0.
...
ANSWER: The mean of CBT is significantly different from the mean of Psychotherapy. Same meaning, now just different direction.
...
ANSWER: intercept: the grand mean of depression. slope: the difference between the groups. Note that it didn't change the *test* from our earlier model, it changed the estimate and the intercept.
...
ANSWER: p-values are lower (some no longer significant) with Bonferroni correction!
...
ANSWER: Whether any mean is different from any other mean.
...
ANSWER: 7.2: CBT 7.2 - 3.8 = 3.4: psychotherapy 7.2 + 2.6 = 9.8: control
...
ANSWER: mean of CBT is significantly different from 0 (because it's the control group).
...
ANSWER: The differences between psychotherapy and control are significantly different from the mean of CBT.
...
ANSWER: mean of the control group; slopes are the differences from the control group.
...
ANSWER: Intercept is the grand mean. Slope for Treat.v.Control is difference between the mean for Control and the mean across the two treatment groups.
