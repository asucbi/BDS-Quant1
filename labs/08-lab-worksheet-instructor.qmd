---
title: "Module 8: Lab Instructions (Instructor File)"
format: html
editor: visual
---
...
cheat <- read_csv("data/new_df.csv")
...
-   Calculate descriptives (i.e., mean and sd) for the variables `honesty` and
    `claimed_solved`
-   Calculate descriptives (i.e., mean and sd) for the variables `honesty` and
...
Notice that the only difference between these two equations are their intercepts. If we graphed the two models, let's see what it would looke like.
...
To get the *simple slopes* and associated signfiicance we can use the `emtrends` function from the `emmeans` package. 

Is the slope for the non-shredder condition significantly different from the slope for the shredder condition?
```{r message= FALSE, echo=FALSE, eval=FALSE}
...
ANSWER: honesty looks negative, confidence is slightly positive but not significant
...
low: $$\hat{claimed_i} = 5.26 + 0.05(-0.87) + (-0.52 - 0.07(-0.87))honesty_c$$ 
$$\hat{claimed_i} = 5.22 - 0.46*honesty_c$$

medium: $$\hat{claimed_i} = 5.26 + 0.05(0) + (-0.52 - 0.07(0))honesty_c$$ 
$$\hat{claimed_i} = 5.26 - 0.52*honesty_c$$

high: $$\hat{claimed_i} = 5.26 + 0.05(0.87) + (-0.52 - 0.07(0.87))honesty_c$$
$$\hat{claimed_i} = 5.30 - 0.58*honesty_c$$
