---
title: "Module 9: Lab Instructions (Instructor)"
format: html
editor: visual
---
...
Plus in different values for combinations into the model. 
...
Drug: H0: pain level is equal across all drugs; H1: at least one drug differs
Dose: H0: pain level is equal across dosages; H1: at least one dosage differs
DrugxDose: H0: the effect of dose does not depend on drug (and vice versa); H1: the effect of dose depends on drug (or vice versa)
...
ANSWER: Yes! In particular, `drug` goes from non-significant (with Type III SS) to significant (with Type I SS). This is because Type I asks how much of the _total variance_ `drug` explains and Type III asks how much of the variance left over after `dose` and `drug x dose` is explained by `drug`.
