...
ggplot(nels, aes(x = XXX, y = XXX, group = XXX)) +
  geom_point(aes(color = as.factor(XXX))) +
  geom_smooth(method = "lm", se = FALSE, aes(group = XXX, color = as.factor(XXX))) +
  theme_minimal() +
  theme(legend.position = "none") +
  labs(title = "Effect of Time on Math on Math Score by School", x = "Time on Math", y = "Math Score")

ggplot(nels, aes(x = XXX, y = XXX, fill = XXX)) +
...
#students: run th likelihood ratio test, use the correct maximum likelihood estimation


test_likelihoodratio(XXX)
...
model_3 <- lmer(mathscore ~ timeonmath_c * schooltype + (XXX), data = nels)
summary(model_3)
...
model_4 <- lmer(mathscore ~ timeonmath_c * schooltype + (XXX), data = nels)
summary(model_4)
...
#semi-R^2
partR2(XXX, data = nels, partvars = c("timeonmath_c"), R2_type = "marginal", nboot = 10, CI = 0.95) 

#cohen's d = effect / sqrt(var(int) + var(slope) + var(resid))
XXX/ sqrt(XXX+XXX+XXX)
