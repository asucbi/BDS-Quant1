...
ggplot(nels, aes(x = schooltype, y = mathscore, fill = schooltype)) +
  ...
  geom_line(aes(group = studentid), alpha = 0.3, color = "gray") + 
  labs(x = "School Type", y = "Math Score",
       title = "Effect of School Type on Math Score") + 
  theme(legend.position = "none") 
...
lrt1 <- test_likelihoodratio(XXX, XXX, REML=FALSE)
...
lrt2 <- test_likelihoodratio(model_2, model_3, REML=TRUE)
lrt2
...
# can pull out fixed and random effects with the `model_parameters()` function!
