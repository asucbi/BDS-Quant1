...
ANSWER: as honesty increases, people claim they solved fewer problems
...
ANSWER: $$\hat{claimed_i} = 6.76 - 0.50honesty $$

> Question: What do each of the parameter estimates mean?

ANSWER: b0: 6.76, predicted number of problems people claim to have solved when honesty = 0
...
ANSWER: $$\hat{claimed_i} = 6.04 - 0.45honesty + 1.12condition$$

> Question: What do each of the parameter estimates mean?

ANSWER: b0: 6.04, predicted number of problems people claim to have solved when `honesty` = 0 and `condition` = 0 (non-shredder condition)

b1: -0.45, for a one-unit change in `honesty`, we predict a 0.45 *decrease* in `claimed_solved` for the non-shredder condition (condition = 0).

b2: 1.12, the difference in `claimed_solved` between the non-shredder and shredder conditions while controlling for `honesty` (when honesty = 0)
...
ANSWER: - Non-shredder (Condition = 0):
...
ggplot(aes(x = honesty, y = claimed_solved)) +
  geom_point() +
  geom_abline(slope = -.45, intercept = 6.04, color = "red", linewidth = 1) +
  geom_abline(slope = -.45, intercept = 7.16, color = "blue", linewidth = 1) +
...
ANSWER: $$\hat{claimed_i} = 4.75 - 0.85honesty_C + 1.12condition + 0.78honest_C*condition$$

> Question: What do each of the parameter estimates mean?

ANSWER: b0: 4.75, the predicted `claimed_solved` for someone with a mean honesty score (honesty_c = 0) and in the non-shredder condition (non-shredder = 0)
...
Mean_Non-shredder: 4.75 - 0.85(0) + 1.12(0) + 0.78(0x0) = 4.75 Mean_Shredder: 4.75 - 0.85(0) + 1.12(1) + 0.78(0x1) = 5.87

These are called *adjusted means*. Another way of obtaining these adjusted means is by using the `emmeans` function.
...
Q1: Is honesty a significant predictor of number of problems people claim to have solved at each level of condition (non-shredder & shredder conditions)? (e.g., is the slope significantly different from zero)
...
ANSWER: $$\hat{claimed_i} = 6.72 - 0.50honesty + 0.01confidence$$

> Question: What do each of the parameter estimates mean?

ANSWER: b0: 6.72, predicted number of problems people claim to have solved when honesty = 0 and confidence = 0 (not centered, so true 0 values)

b1: -0.50, for a one-unit increase in `honesty`, we expect a decrease of 0.50 in `claimed_solved`, holding confidence constant

b2: 0.01, for one-unit increase in confidence, we expect an increase of 0.01 in `claimed_solved`, holding `honesty` constant.
...
ANSWER: honesty looks negative, confidence is slightly positive but not likely to be significant
...
> Question: Is there multicollinearity? What could you do to fix it?

ANSWER: Yes -- Centering!
...
ANSWER: $$\hat{claimed_i} = 5.27 - 0.52honesty_c + 0.05confidence_c - 0.07honesty_c*confidence_c$$

> Question: What do each of the parameter estimates mean?

ANSWER: b0: 5.27, the predicted `claimed_solved` for someone with a mean `honesty` score (honesty_c = 0) and mean `confidence` (confidence_c = 0) score.

b1: -0.52, for every 1-unit increase in `honesty_c`, the model predicts a *decrease* in `claimed_solved` of 0.52 when `confidence_c` is 0 (at its mean).

b2: 0.05, for every 1-unit increase in `confidence_c`, the model predicts an *increase* in `claimed_solved` of 0.05 when `honesty_c` is 0 (at its mean).

b3: -0.07, the interaction between `honesty_c` and `confidence_c`. the difference in slope of `honesty_c` for a 1-unit increase in `confidence_c`. For a one-unit increase in `confidence_c` there is a -0.07 change in the slope between `honesty_c` and `claimed_solved`
...
ANSWER: $$\hat{claimed_i} = 5.26 + 0.05confidence_c + (-0.52 - 0.07confidence_c)honesty_c$$
...
low: $$\hat{claimed_i} = 5.26 + 0.05(-0.87) + (-0.52 - 0.07(-0.87))honesty_c$$ $$\hat{claimed_i} = 5.22 - 0.46*honesty_c$$

medium: $$\hat{claimed_i} = 5.26 + 0.05(0) + (-0.52 - 0.07(0))honesty_c$$ $$\hat{claimed_i} = 5.26 - 0.52*honesty_c$$

high: $$\hat{claimed_i} = 5.26 + 0.05(0.87) + (-0.52 - 0.07(0.87))honesty_c$$ $$\hat{claimed_i} = 5.30 - 0.58*honesty_c$$
...
ANSWER: high and medium differ, low doesn't differ from zero
...
ANSWER: with high confidence, as `honesty_c` increases, `claimed_solved` decreases; with medium confidence, the same direction of effect happens (decrease), but the slope isn't as steep; with low confidence, the slope is much flatter suggesting that `honesty_c` has less of an effect on `claimed_solved`. In fact, the effect of `honesty_c` on `claimed_solved` is not significant at low levels of `confidence_c`. we also see a crossover interaction where, at low levels of `honesty_c` those with high confidence have the highest values of `claimed_solved` while, at higher levels of `honesty_c` those with low confidence have the highest values of `claimed_solved`, though there is still a general decrease in `claimed_solved` with increasing honesty.
