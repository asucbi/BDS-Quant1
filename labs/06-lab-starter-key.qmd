---
title: "Module 6: Lab Starter - KEY"
format: html
editor: visual
---
...
```{r examine-data, echo=FALSE, eval=FALSE, message=FALSE}

str(cue_data)

# we might want to make `Condition` a factor -- do that!
# students: fill in code to make it a factor 

# cue_data <- cue_data %>% 
#   mutate(Condition = XXX)
...
summary(model)
# interpret overall and individual effects
...
# Look what happens if we don't adjust for multiple comparisons (compare this to your summary)
emmeans(model, pairwise ~ Condition, adjust = "none")
...
Which equation for SD should we use?
...
# students: fill in these equations
...
cue_overt <- (.500-.524)/ sqrt( ((43 - 1) * 0.168 ^ 2 + (40 - 1) * .135 ^ 2) / (43 + 40 - 2))

cue_restudy <- (.500-.437)/ sqrt( ((43 - 1) * 0.168 ^ 2 + (40 - 1) * .169 ^ 2) / (43 + 40 - 2))

overt_restudy <- (.524-.437)/ sqrt((0.135^2 + .169^2)/2)
...
# Heuristics for Cohen's d: 0.20 = S, 0.50 = M, 0.80 = L
...
Let's first check out the `d_ind_t` function. What information do you need to provide?

```{r effect-ci}

?d_ind_t

# students: fill in arguments

# cue_overt_d <- d_ind_t(XXX)
# 
# cue_restudy_d <- d_ind_t(XXX)
#                     
# overt_restudy_d <- d_ind_t(XXX)
...
cue_overt_d <- d_ind_t(m1 = .500, m2 = .524, .168, .135, 43, 40)

cue_restudy_d <- d_ind_t(.500, .437, .168, .169, 43, 40)
                    
overt_restudy_d <- d_ind_t(.524, .437, .135, .169, 40, 40)

cue_overt_d$estimate

cue_restudy_d$estimate

overt_restudy_d$estimate
...
# first, let's look at the help info for `wp.regression`
?wp.regression

# students: fill in the arguments below

# WebPower::wp.regression(n=XXX, p1=XXX, f2=XXX, alpha=XXX, power=XXX)
...
WebPower::wp.regression(n=200, p1=2, f2=NULL, alpha=0.05, power=.9)
...
#first, let's look at what we need for this function
?ANOVA_design
