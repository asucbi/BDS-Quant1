---
title: "Module 7: Lab Instructions (Instructor File)"
format: html
editor: visual
---
...
INST ANS: they look pretty different; zero isn't a meaningful number; "winter" looks quite a bit different ini scale than the others(mean and sd)
...
INST ANS: predicted population for a state with 0 average income; not meaningful (population is negative!)
...
INST ANS: predicted population for a state with an average income; much more meaningful!

> **Question: How would you interpret the slope?**

INST ANS: predicted increase in population for each one-unit increase in income
...
INST ANS: predicted population for a state with an average salary and 0 degree average winter temperature -- tricky! you might want to center `winter` too!

> **Question: How would you interpret the slope for income_c?**

INST ANS: predicted increase in population for each one-unit increase in income, holding winter temp constant; significant!

> **Question: How would you interpret the slope for winter?**

INST ANS: predicted increase in population for each one-unit increase in winter temp, holding income constant; significant!

> **Question: How would you summarize your findings?**

INST ANS: both income and winter temp positively predict population; states with higher income have larger populations and states with higher winter temperatures have larger populations
...
INST ANS: predicted population for a state at mean age and mean winter temp

> **Question: How would you interpret the effect of income?**

INST ANS: for each 1SD increase in income, we expect population to increase by 1977

> **Question: How would you interpret the effect of winter?**

INST ANS: for each 1SD increase in winter temp, we expect population to increase by 2465
...
INST ANS: intercept and slope (intercept because last time we only centered income), overall model fit, t-values, and p-values stay the same
...
INST ANS: population, because it is skewed
...
INST ANS: more normally distributed
...
INST ANS: put the outcome variable back into original units

Recall that:

$log_b(n) = x \leftrightarrow b^x = n$

To interpret the intercept, use this to put the outcome variable back into original units.
...
INST ANS: at zero average temperature, the predicted population is 832
...
For every one-unit increase in our predictor, our dependent variable increases/decreases by this amount.
...
#subtract one from this number, multiply by 100
(winter_exp-1)*100
...
INST ANS: for each one-unit increase in winter temperature, we expect population to increase by 4.16%
...
INST ANS: "increase in conscientiousness with age"

> **Question: Which part of the above paragraph refers to a *quadratic* trend of age?**

INST ANS: "greatest during early adulthood (20s and 30s) and slow thereafter" -- suggests a curve
...
```{r read-and-view2, message= FALSE, echo=FALSE, eval=FALSE}
...
INST ANS: does seem to increase and then start to flatten out a little bit
...
INST ANS: predicted conscientiousness for someone at age 0

> **Question: How would you interpret the slope?**

INST ANS: predicted increase in conscientiousness for each one-unit increase in age
...
INST ANS: Both linear and quadratic trends.
...
INST ANS: Quadratic, it's the most parsimonious model that explains the data.
...
INST ANS: Still quadratic -- it improves fit over linear and increases r squared by 0.03; cubic doesn't improve fit much over quadratic
...
INST ANS: There are both linear and quadratic effects.
...
-   report linear effect ( *b*, *SE*, *t* statistic with associated *df*, 95%CI, *p*-value)
-   interpret linear effect (e.g., as x increases/decreases, y increases/decreases)
-   report quadratic effect ( *b*, *SE*, *t* statistic with associated *df*, 95%CI, *p*-value)
